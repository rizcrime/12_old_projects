...
class M_admin extends CI_Model {
	...
	public function select_all() {
		...
		if ( $id_role=='2' || $id_role =='10' || $id_role =='16' || $id_role =='17' || $id_role =='20'){
		...
		else if ( $id_role=='3' || $id_role =='11' || $id_role =='15' || $id_role =='18' || $id_role =='21'){
			// $this->db->where('ROLE <>', 'KLIK');
			// ->where('ROLE <>', 'KLIK');
		...
		else if ( $id_role=='4' || $id_role =='12' || $id_role =='13' || $id_role =='19' || $id_role =='22'){
			// $this->db->where('ROLE <>', 'KLIK');
			// ->where('ROLE <>', 'KLIK');
		...
		// $id_role = 2;
		// echo $id_role.'hheTEST';
		// echo $role_pusdatin.'1';
		// echo $role_berita.'2';
		// echo $role_geologi.'3';
		...
		// if ( $id_role=='1'){
		// 	// $this->db->where('ROLE <>', 'KLIK');
		// 	// $this->db->where('t_user.ID_ROLE', $id_role);	
	...
	public function select_user_is_post($id) {
		$this->db->select('
			t_user.IS_POST,t_user.IS_REVIEW,t_user.IS_ENTRY,t_user.READY_REVIEW,t_user.HAS_REVIEW,t_user.READY_POST,t_user.HAS_POST,t_user.HAS_UNPOST
		');
		$this->db->from('t_user');
		$this->db->where('t_user.ID_USER', $id);
		$data = $this->db->get();
...
// echo $data['USERNAME'];
// // echo $data['IS_POST'];
// $isPost = isset($data['IS_POST']) ? $isPost = $data['IS_POST'] ;
// echo $isPost.'111';
// 		die();
		$val = array(
			...
			// 'IS_POST' => $isPost,
			'IS_REVIEW' => (isset($data['IS_REVIEW']) ? $isReview = $data['IS_REVIEW'] : $isReview = "T" ),
			'IS_POST' => (isset($data['IS_POST']) ? $isPost = $data['IS_POST'] : $isPost = "T" ),
			'IS_ENTRY' => (isset($data['IS_ENTRY']) ? $isEntry = $data['IS_ENTRY'] : $isEntry = "T" )
		);
