...
foreach ($dataEntry as $entry) {
  ...
  <tr>
    ...
    <?php if($IS_ENTRY == 'Y'){ ?>
    	<button class="btn btn-warning update-dataDraftOpec btn-minier" data-id="<?php echo $entry->ID_LAPORAN; ?>" data-jenis="<?php echo 8 ?>"><i class="glyphicon glyphicon-repeat"></i> Update</button>
    <?php } ?>
...
foreach ($dataSet as $data) {
  ...
  <tr>
    ...
    <?php if($IS_ENTRY == 'Y'){ ?>
    	<button class="btn btn-warning update-dataDraftOpec btn-minier" data-id="<?php echo $data->ID_LAPORAN; ?>" data-jenis="<?php echo 8 ?>"><i class="glyphicon glyphicon-repeat"></i> Update</button>
    <?php } ?>
    
     <?php if($IS_REVIEW == 'Y'){ ?>
    	<button class="btn btn-primary  review-dataDraftOpec btn-minier" data-id="<?php echo $data->ID_LAPORAN; ?>" data-jenis="<?php echo 8 ?>" ><i class="glyphicon fa fa-search" ></i> Review</button>
    <?php } ?>
...
<!--      <a target="_blank" href="<?php echo base_url().'Lap_pusdatin/downloadFileTextBeritaOpecHarian/'.$data->ID_LAPORAN;?>">Download</a>
-->  </td>
</tr>
<?php
$no++;
