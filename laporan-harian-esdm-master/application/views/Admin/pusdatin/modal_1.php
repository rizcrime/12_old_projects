...
<?=get_csrf_token()?>-->
			<div class="main-container ace-save-state" id="laporan1">
...
<?=get_csrf_token()?>						<div class="row">
							<div class="col-xs-12 col-sm-9">
								<div class="profile-user-info profile-user-info-striped">
									<div class="profile-info-row">
										...
										<div class="profile-info-value">
											<div class="input-group">
                                                ...
                                                <button  type="button" class="btn btn-success btn-minier" onclick="ProdMinyakGet()" id="select-gunung">Show Before</button> 
											</div>
										</div>
									</div>

									<div class="profile-info-row">
										<div class="profile-info-name"> Produksi Minyak Harian (BOPD) </div>
									...
									<div class="profile-info-row">
										<div class="profile-info-name"> Rata - rata Produksi Bulanan (BOPD) </div>
									...
									<div class="profile-info-row">
										<div class="profile-info-name"> Rata - rata Produksi Tahunan (BOPD) </div>
									...
									<div class="profile-info-row">
										<div class="profile-info-name"> Target APBN (BOPD) </div>

										<!--<div class="profile-info-value">
											<input required class="form-control" type="number" name="APBN" id="APBN" value="775000" readonly>
										</div>-->
    ...
    $dataTahunTargetProdMinyak = $this->db->query("SELECT APBN_PROD_MINYAK from t_target where TAHUN = '".date('Y')."' ORDER BY ID_TARGET DESC limit 1");
      $no=1;
      foreach ($dataTahunTargetProdMinyak->result_array() as $row)
    {
    ?>
											<input required class="form-control" type="number" name="APBN" id="APBN" value="<?php echo $row['APBN_PROD_MINYAK'] ?>" readonly>
                                            <?php } ?>
										</div>
									</div>
...
// Submit upload
    $('#upload').submit(function(e) {
        ...
        $.ajax({
            ...
            url: '<?php echo base_url('Lap_pusdatin/prosesUploadProdMinyak'); ?>',
            data: formData,
            processData: false,
            contentType: false
        })
...
function ProdMinyakGet(){
	...
	$.ajax({
		...
		url: "<?php echo site_url('Lap_pusdatin/showBeforeDateProdMinyak'); ?>",
		data: {
			"tanggal": tanggal
		},
