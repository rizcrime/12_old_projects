...
<?php */?>            
	...
	<?php }
	else if( $userdata->IS_REVIEW == "Y"  && $READY_REVIEW == 'Y' &&  $HAS_REVIEW == ''  && $userdata->IS_POST == "T" &&  $userdata->IS_ENTRY == "T" ) { ?>
...
<!--            <li><a href="#4" data-toggle="modal" data-target="#view-all-has-unposting">View All Draft</a>
	...
	<?php }
	 else if( $userdata->IS_REVIEW == "T" &&  $userdata->IS_POST == "Y" && $READY_POST == "Y" && $HAS_POST == "" &&  $userdata->IS_ENTRY == "T") { ?>
...
<!--            <li><a href="#5" data-toggle="modal" data-target="#konfirmasiUnpost">Unposting All</a>
	...
	<?php }
	 else if( $userdata->IS_REVIEW == "T" &&  $userdata->IS_POST == "Y" && $READY_POST == ''  && $HAS_POST == "Y"  &&  $userdata->IS_ENTRY == "T") { ?>
		<ul class="nav nav-tabs">
			<!--<li class="active">
        <a  href="#1" data-toggle="tab">Entry Laporan</a>
			</li>-->
	...
	<?php }
		...
		<form method="post" id="import_for" action="<?php echo base_url('Lap_geologi/upload_all_excel'); ?>" enctype="multipart/form-data">
			<?=get_csrf_token()?>
...
-->												</div>
												...
												<!--<div class="profile-info-row">
													<div class="profile-info-name"> Tugas Produksi Minyak(<?php echo $notif_tugas_entry[0]->r_lap_geologi_prod_minyakCount ?>)</div>
													<div class="profile-info-name">Tugas ICP(<?php echo $notif_tugas_entry[0]->r_lap_geologi_icpCount ?>)
													</div>
													<div class="profile-info-name"> Tugas Produksi Gas(<?php echo $notif_tugas_entry[0]->r_lap_geologi_prod_gasCount ?>)</div>
													<div class="profile-info-name">Tugas Produksi Ekuivalen Minyak & Gas(<?php echo $notif_tugas_entry[0]->r_lap_geologi_prod_ekui_migasCount ?>)
													</div>
											...
											<!--	<div class="profile-info-row">
													<div class="profile-info-name"> Tugas Lifting Tahun Berjalan (<?php echo $notif_tugas_entry[0]->r_lap_geologi_lift_tbCount ?>)</div>
													<div class="profile-info-name"> Tugas Harga BBM (<?php echo $notif_tugas_entry[0]->r_lap_geologi_harga_bbmCount ?>)
													</div>
													
													<div class="profile-info-name">Tugas Berita Opec Harian (<?php echo $notif_tugas_entry[0]->r_lap_geologi_berita_opec_harianCount ?>)
													</div>
													<div class="profile-info-name"> Tugas Harga Batubra Acuan (<?php echo $notif_tugas_entry[0]->r_lap_geologi_harga_bb_acuanCount ?>)</div>
													
												</div>-->
												<!--<div class="profile-info-row">
													
													<div class="profile-info-name">Tugas Harga Mineral Acuan (<?php echo $notif_tugas_entry[0]->r_lap_geologi_harga_mineral_acuanCount ?>)
													</div>
													<div class="profile-info-name">Tugas Status Ketenagalistrikan	 (<?php echo $notif_tugas_entry[0]->r_lap_geologi_stts_tlCount ?>)
													</div>
...
<?php /*?><?= $modal_view_all_has_unposting; ?>

<?= $modal_view_all_kapus; ?><?php */?>
...
$(document).ready(function(){
	...
	{
		$.ajax({
			url:"<?php echo base_url(); ?>Lap_geologi/upload",
			method:"POST",
...
<script>
  ...
  function tampilAdmin() {
	...
	$("#ID_JENIS_LAPORAN").change(function() {
		...
		$('#tempat-modal').html('');
		$.get('<?php echo base_url('Lap_geologi/show_form'); ?>/'+ID_JENIS_LAPORAN, function(data) {
	...
	$("#btnTampilkan").click(function() {
		...
		$.get('<?php echo base_url('Lap_geologi/show_form_history'); ?>/'+ID_JENIS_LAPORAN+'/'+TANGGAL_AWAL+'/'+TANGGAL_AKHIR, function(data) {
			MyTable2.fnDestroy();
  ...
  $(document).on("click", ".review-dataDraft", function() {
    ...
    $.ajax({
      method: "POST",
      url: "<?php echo base_url('Lap_geologi/hasreview'); ?>",
  ...
  $(document).on("click", ".konfirmasiHapus-target", function() {
	...
	$(document).on("click", ".hapus-dataTarget", function() {
		...
		$.ajax({
			method: "POST",
			url: "<?php echo base_url('Lap_geologi/delete'); ?>",
			//data: "id=" +id
			data: "id=" +id + "&ID_JENIS_LAPORAN=" +ID_JENIS_LAPORAN,
		})
		.done(function(data) {
  ...
  $(document).on("click", ".post-dataAdmin", function() {
    ...
    $.ajax({
      method: "POST",
      url: "<?php echo base_url('Lap_geologi/post'); ?>",
      data: "id=" +id + "&ID_JENIS_LAPORAN=" +ID_JENIS_LAPORAN,
    })
    .done(function(data) {
  ...
  $(document).on("click", ".post-dataDraft", function() {
   ...
   $(document).on("click", ".Unpost-dataDraft", function() {
    $.ajax({
      method: "POST",
      url: "<?php echo base_url('Lap_geologi/UnpostAll'); ?>",
    })
    .done(function(data) {
 ...
 $(document).on("click", ".review-dataDraftAll", function() {
    $.ajax({
      method: "POST",
      url: "<?php echo base_url('Lap_geologi/hasreviewall'); ?>",
    })
    .done(function(data) {
