...
-->                                                <input class="form-control date-picker" name="TANGGAL_LAPORAN" id="id-date-picker-1" type="text" value="<?php echo $kemarin?>" data-date-format="dd-mm-yyyy" disabled />
                                                <button  type="button" class="btn btn-success btn-minier" onclick="GunungApiGet()" >Show Before</button>  
											</div>
										</div>
									</div>
									...
									<div class="profile-info-row">
										...
										<div class="profile-info-value">
											...
											<select required class="form-control" name="ID_GUNUNG" id="ID_GUNUNG">
									...
									<div class="profile-info-row">
										...
										<div class="profile-info-value">
											<select required class="form-control" name="ID_AKTIVITAS" id="ID_AKTIVITAS">
												<option>---Pilih---</option>
									...
									<div class="profile-info-row">
										...
										<div class="profile-info-value">
											<textarea required class="form-control" type="text" name="KETERANGAN" id="KETERANGAN" placeholder="Seismograf" style="resize: vertical;"></textarea>
										</div>
									</div>
									...
									<div class="profile-info-row">
										...
										<div class="profile-info-value">
											<textarea required class="form-control" type="text" name="REKOMENDASI" id="REKOMENDASI" placeholder="Rekomendasi" style="resize: vertical;"></textarea>
										</div>
									</div>
									...
									<div class="profile-info-row">
										...
										<div class="profile-info-value">
											<textarea required class="form-control" type="text" name="VONA" id="VONA" placeholder="Vona" style="resize: vertical;"></textarea>
										</div>
									</div>
									...
									<div class="profile-info-row">
										...
										<div class="profile-info-value">
											<textarea required class="form-control" type="text" name="DETAIL" id="DETAIL" placeholder="Detail" style="resize: vertical;"></textarea>
										</div>
									</div>
								</div>
...
// Submit upload
    $('#upload').submit(function(e) {
        ...
        $.ajax({
            ...
            url: '<?php echo base_url('Lap_geologi/prosesUploadGunungApi'); ?>',
            data: formData,
            processData: false,
            contentType: false
        })
        .done(function(data) {
            ...
            if (out.status == true) {
			  // $('#ID_GUNUNG').val(out.ID_GUNUNG);
			   $('select[name="ID_GUNUNG"]').val(out.ID_GUNUNG);
			   
               //$("#ID_AKTIVITAS").val(out.ID_AKTIVITAS);
			   $('select[name="ID_AKTIVITAS"]').val(out.ID_AKTIVITAS);
             //  $("#REKOMENDASI").val(out.REKOMENDASI);
			   $('textarea[name="REKOMENDASI"]').val(out.REKOMENDASI);
             //  $("#VONA").val(out.VONA);
			   $('textarea[name="VONA"]').val(out.VONA);
			  // $("#DETAIL").val(out.DETAIL);
			    $('textarea[name="DETAIL"]').val(out.DETAIL);
			  // $("#KETERANGAN").val(out.KETERANGAN);
			   $('textarea[name="KETERANGAN"]').val(out.KETERANGAN);
            } else {
                alert(out.errorMsg);
            }
        })
