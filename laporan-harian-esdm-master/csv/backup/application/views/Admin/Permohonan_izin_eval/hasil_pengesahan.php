        ...
        <div class="input-group form-group" style="width: 100%; background-color: white; padding: 0 0px 10px 0px; border: solid 1px #3c8dbc; border-radius: 5px;">
          ...
          <div class="col-md-12">
            ...
            <?php elseif($pengesahan_data->status == "success"): ?>
              ...
              <h4 class="text-center">File Izin: <a href='<?=$pengesahan_data->urlIzin?>' target='_blank'><?=$pengesahan_data->fileIzin?></a></h4>
            <?php else: ?>
              <h1 class="text-center"><?=$pengesahan_data->message?></h1>
            <?php endif; ?>
          </div>
          <div class="col-md-12">
            <a href='<?=base_url("Permohonan_izin_eval")?>'><button class="btn btn-primary pull-right" style="margin-right: 5px">Periksa Izin Lain</button></a>
          </div>
        </div>
    </div>
