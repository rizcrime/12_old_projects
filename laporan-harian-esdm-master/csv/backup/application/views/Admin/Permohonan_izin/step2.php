...
<div class="row">
  <div class="box-body">
    ...
    <div class="widget-main">
      <div id="fuelux-wizard-container">
        <div>
          <ul class="steps">
            <li data-step="1">
              <a href="<?php echo site_url('Permohonan_izin/step1') ?>" style="display: block;">
    ...
    <form id="main-form" method="POST" action="<?php echo base_url('Permohonan_izin/step3') ?>">
    <?=get_csrf_token()?>
     <div class="input-group form-group" style="width: 100%; background-color: white; padding: 0 0px 10px 0px; border: solid 1px #3c8dbc; border-radius: 5px;">
      ...
      <div class="col-md-12">
        <select name="ID_FORM" ID="IZIN_INSTANSI" class="form-control" required>
          <option value="">PILIH</option>
          <?php foreach ($dataIzinInstansi as $row): ?>
            <option value="<?= $row->ID_FORM ?>"><?= $row->NAMA_FORM ?></option>          
          <?php endforeach ?>
        </select>
      </div>
      <div class="col-md-12">Jenis</div>
      <div class="col-md-12">
        <select name="ID_TEMPLATE" class="TEMPLATE form-control" onchange="f_show(this)" required>
          <option value="">PILIH</option>
        </select>
      </div>

      <?php foreach ($dataTemplate as $parent): ?>
	  <div id="prasyarat<?php echo $parent->ID_TEMPLATE ?>" class="col-md-12" style="display: none">
	  		<table class="table table-bordered table-striped" style="padding: 10px; border: solid 1px #c0d9ec;">
	            ...
	            <tbody>
					<tr><td><?php echo $dataPrasyarat[$parent->ID_TEMPLATE] ?></td></tr>
	            </tbody>
	      	</table>
	  </div>
      ...
      <div class="col-md-12">
        <input type="checkbox" name="" required> &nbsp; Saya menyetujui <a data-toggle="modal" data-target="#form-sk"  href="" style="color:blue;">syarat dan ketentuan</a> ini.
      </div>
    </div>
    <div style="overflow:auto;">
      ...
      <div style="float:right;">
        <a class="btn-sm btn-info" href="<?php echo site_url('Permohonan_izin/step1') ?>" style="color: white"><i class="fa fa-chevron-left"></i> Kembali</a>
        <button type="submit" id="izin-next-step" class="btn-sm btn-success" onclick="nextPrev(1)">Lanjut <i class="fa fa-chevron-right"></i></button>
      </div>
    </div>
  </form>
</div>
...
<script type="text/javascript">
  ...
  function f_show(obj) {
    if(obj.value){
      ...
      $('div[id^=prasyarat]').attr("style","display:none");
      $("#prasyarat"+obj.value).attr("style","display:block");
    } else {        
      $('div[id^=dokumen]').attr("style","display:none");
      $('div[id^=prasyarat]').attr("style","display:none");
    }
