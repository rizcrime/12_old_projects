...
foreach ($dataAkta as $akta) {
  ...
  <tr>
    ...
    <?php if(!empty($akta->FILE_AKTA)): ?>
      <td><a href='<?=base_url("Profile_perusahaan/download_dokumen/akta?file={$akta->ID_AKTA}")?>' target="_blank"><?php echo $akta->NOMOR_AKTA; ?></a></td>
    <?php else: ?>
    ...
    <?php if(!empty($akta->FILE_PENGESAHAN)): ?>
      <td><a href='<?=base_url("Profile_perusahaan/download_dokumen/pengesahan?file={$akta->ID_AKTA}")?>' target="_blank"><?php echo $akta->NO_PENGESAHAN; ?></a></td>
    <?php else: ?>
