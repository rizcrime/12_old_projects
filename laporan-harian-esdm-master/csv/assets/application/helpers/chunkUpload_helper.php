...
class UploadHandler {
    ...
    public function combineChunks($uploadDirectory, $name = null) {
        ...
        for ($i=0; $i<$totalParts; $i++){
            try {
                $fileName = $targetFolder.DIRECTORY_SEPARATOR.$i;
...
function getUploaderInstance($id_perusahaan, $allowedExtensions = NULL, $sizeLimit = null) {
    if(is_null($allowedExtensions)) {
        $_ci = &get_instance();
        $_ci->load->config('allowed_extension');
        $allowedExtensions = $_ci->config->item("allowed_extension");
    }
    ...
    $uploader->chunksFolder = "FILE_UPLOAD/".$id_perusahaan."/CHUNKS";

    if(!is_dir($uploader->chunksFolder)){
        mkdir($uploader->chunksFolder, 0757, true);
    }
